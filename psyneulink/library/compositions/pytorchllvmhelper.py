...
def gen_inject_vec_binop(ctx, builder, op, u, v, output_vec=None):
    ...
    if output_vec is None:
        output_vec = builder.alloca(u.type.pointee, name="binop_output_vector")
...
def gen_inject_mat_binop(ctx, builder, op, m1, m2, output_mat=None):
    ...
    if output_mat is None:
        output_mat = builder.alloca(m1.type.pointee, name="binup_output_matrix")
...
def gen_inject_mat_scalar_mult(ctx, builder, m1, s, output_mat=None):
    ...
    if output_mat is None:
        matrix_type = pnlvm.ir.types.ArrayType(pnlvm.ir.types.ArrayType(ctx.float_ty, y), x)
        output_mat = builder.alloca(matrix_type, name="MxS_output_matrix")
...
def gen_inject_vxm(ctx, builder, v, m2, output_vec=None):
    ...
    # create output vec
    if output_vec is None:
        output_vec = builder.alloca(pnlvm.ir.types.ArrayType(ctx.float_ty, z), name="VxM_output_vector")
...
def gen_inject_vxm_transposed(ctx, builder, v, m2, output_vec=None):
    ...
    # create output vec
    if output_vec is None:
        output_vec = builder.alloca(pnlvm.ir.types.ArrayType(ctx.float_ty, y), name="MxV_output_vector")
