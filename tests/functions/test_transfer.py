...
test_data = [
    ...
    pytest.param(pnl.Exponential, test_var, {kw.SCALE: RAND1, kw.RATE: RAND2}, RAND1 * np.exp(RAND2 * test_var),
                 id="EXPONENTIAL"),
    pytest.param(pnl.Logistic, test_var, {kw.GAIN: RAND1, kw.X_0: RAND2, kw.OFFSET: RAND3, kw.SCALE: RAND4},
                 logistic_helper, id="LOGISTIC"),
    pytest.param(pnl.Tanh, test_var, {kw.GAIN: RAND1, kw.BIAS: RAND2, kw.X_0: RAND3, kw.OFFSET: RAND4}, tanh_helper,
                 id="TANH"),
    pytest.param(pnl.ReLU, test_var, {kw.GAIN: RAND1, kw.BIAS: RAND2, kw.LEAK: RAND3}, relu_helper, id="RELU"),

    pytest.param(
        pnl.HypersphericalToCartesian,
        [0.0],
        {},
        [1.0, 0.0],
        id="HYPERSPHERICAL_TO_CARTESIAN_S1_ZERO",
    ),
    pytest.param(
        pnl.HypersphericalToCartesian,
        [np.pi / 2],
        {},
        [0.0, 1.0],
        id="HYPERSPHERICAL_TO_CARTESIAN_S1_RIGHT_ANGLE",
    ),
    pytest.param(
        pnl.HypersphericalToCartesian,
        [0.0, 0.2],  # second angle irrelevant at the north pole
        {},
        [1.0, 0.0, 0.0],
        id="HYPERSPHERICAL_TO_CARTESIAN_S2_NORTH_POLE",
    ),
    pytest.param(
        pnl.HypersphericalToCartesian,
        [np.pi, 0.7],  # second angle irrelevant at the south pole
        {},
        [-1.0, 0.0, 0.0],
        id="HYPERSPHERICAL_TO_CARTESIAN_S2_SOUTH_POLE",
    ),
    ...
    pytest.param(
        ...
        {},
        [0.0, 0.0, 1.0],
        id="HYPERSPHERICAL_TO_CARTESIAN_S2_POS_Z",
    ),

    # Distort
    pytest.param(pnl.Gaussian, test_var,
                 {kw.STANDARD_DEVIATION: RAND1, kw.BIAS: RAND2, kw.SCALE: RAND3, kw.OFFSET: RAND4}, gaussian_helper,
                 id="GAUSSIAN"),
    pytest.param(pnl.GaussianDistort, test_var, {kw.BIAS: RAND1, kw.VARIANCE: RAND2, kw.OFFSET: RAND3, kw.SCALE: RAND4},
                 gaussian_distort_helper(0), id="GAUSSIAN DISTORT GLOBAL SEED"),
    pytest.param(pnl.GaussianDistort, test_var,
                 {kw.BIAS: RAND1, kw.VARIANCE: RAND2, kw.OFFSET: RAND3, kw.SCALE: RAND4, 'seed': 0},
                 gaussian_distort_helper(0), id="GAUSSIAN DISTORT"),
    pytest.param(pnl.BinomialDistort, test_var, {'seed': 0, 'p': RAND1}, binomial_distort_helper(0),
                 id="BINOMIAL DISTORT"),
    ...
    pytest.param(pnl.SoftMax, [test_var, test_var], {kw.GAIN: RAND1, kw.PER_ITEM: True}, softmax_helper2,
                 id="SOFT_MAX ALL PER_ITEM"),
    pytest.param(pnl.SoftMax, [test_var, test_var], {kw.GAIN: RAND1, kw.OUTPUT_TYPE: pnl.ARG_MAX, kw.PER_ITEM: True},
                 np.where(softmax_helper2 == np.max(softmax_helper2), softmax_helper2, 0),
                 id="SOFT_MAX ARG_MAX PER_ITEM"),
    pytest.param(pnl.SoftMax, [test_var, test_var],
                 {kw.GAIN: RAND1, kw.OUTPUT_TYPE: pnl.ARG_MAX_INDICATOR, kw.PER_ITEM: True},
    ...
    pytest.param(pnl.SoftMax, [test_var, test_var], {kw.GAIN: RAND1, kw.OUTPUT_TYPE: kw.MAX_VAL, kw.PER_ITEM: True},
                 np.where(softmax_helper2 == np.max(softmax_helper2), softmax_helper2, 0),
                 id="SOFT_MAX MAX_VAL PER_ITEM"),
    pytest.param(pnl.SoftMax, [test_var, test_var],
                 {kw.GAIN: RAND1, kw.OUTPUT_TYPE: kw.MAX_INDICATOR, kw.PER_ITEM: True},
    ...
    pytest.param(pnl.SoftMax, test_var,
                 {kw.GAIN: RAND1, 'mask_threshold': RAND1 * .5, kw.OUTPUT_TYPE: pnl.ARG_MAX_INDICATOR,
                  kw.PER_ITEM: False},
    ...
    # Linear Matrix
    pytest.param(pnl.MatrixTransform, test_var, {kw.MATRIX: test_matrix}, np.dot(test_var, test_matrix),
                 id="LINEAR_MATRIX SQUARE"),
    pytest.param(pnl.MatrixTransform, test_var, {kw.MATRIX: test_matrix_l}, np.dot(test_var, test_matrix_l),
                 id="LINEAR_MATRIX WIDE"),
    pytest.param(pnl.MatrixTransform, test_var, {kw.MATRIX: test_matrix_s}, np.dot(test_var, test_matrix_s),
                 id="LINEAR_MATRIX TALL"),
...
derivative_test_data = [
    ...
    (pnl.Logistic, test_var, {kw.GAIN: RAND1, kw.X_0: RAND2, kw.OFFSET: RAND3, kw.SCALE: RAND4},
     RAND1 * RAND4 * logistic_helper * (1 - logistic_helper)),
    (pnl.ReLU, test_var, {kw.GAIN: RAND1, kw.BIAS: RAND2, kw.LEAK: RAND3},
     np.where((test_var - RAND2) > 0, RAND1, RAND1 * RAND3)),
    ...
    pytest.param(pnl.SoftMax, test_var, {kw.GAIN: RAND1, kw.OUTPUT_TYPE: kw.ALL, kw.PER_ITEM: False},
                 [[0.088635686173821480, -0.010058549286956951, -0.009789214523259433, -0.009653377599514660,
                   -0.009374948470179183,
                   -0.009891677863509920, -0.009406534609578588, -0.010496622361458180, -0.010680386821751540,
                   -0.009284374637613039],
                  [-0.010058549286956951, 0.091856076128865180, -0.010190413769852785, -0.010049009732287338,
                   -0.009759169518165271,
                   -0.010297076447528582, -0.009792050177702091, -0.010926813872042194, -0.011118109698906910,
                   -0.009664883625423075],
                  [-0.009789214523259433, -0.010190413769852785, 0.089669339130699100, -0.009779930406389987,
                   -0.009497851156931268,
                   -0.010021354713444461, -0.009529851380888969, -0.010634229847424508, -0.010820403403188785,
                   -0.009406089929318929],
                  [-0.009653377599514660, -0.010049009732287338, -0.009779930406389987, 0.088560779144081720,
                   -0.009366057244326959,
                   -0.009882296570138368, -0.009397613427348460, -0.010486667337129447, -0.010670257514724050,
                   -0.009275569312222474],
                  [-0.009374948470179183, -0.009759169518165271, -0.009497851156931268, -0.009366057244326959,
                   0.08627659236704915,
                   -0.009597264807784339, -0.009126561218167337, -0.010184203911638403, -0.010362498859374313,
                   -0.009008037180482098],
                  [-0.009891677863509920, -0.010297076447528582, -0.010021354713444461, -0.009882296570138368,
                   -0.009597264807784339,
                   0.090503011588098000, -0.009629599976882700, -0.010745537931292683, -0.010933660158663310,
                   -0.009504543118853646],
                  [-0.009406534609578588, -0.009792050177702091, -0.009529851380888969, -0.009397613427348460,
                   -0.009126561218167337,
                   -0.009629599976882700, 0.086536526770559590, -0.010218516599910580, -0.010397412260182810,
                   -0.009038387119898062],
                  [-0.010496622361458180, -0.010926813872042194, -0.010634229847424508, -0.010486667337129447,
                   -0.010184203911638403,
                   -0.010745537931292683, -0.010218516599910580, 0.095380732590004670, -0.011602329078808723,
                   -0.01008581165029997],
                  [-0.010680386821751540, -0.011118109698906910, -0.010820403403188785, -0.010670257514724050,
                   -0.010362498859374313,
                   -0.010933660158663310, -0.010397412260182810, -0.011602329078808723, 0.096847441839448930,
                   -0.010262384043848514],
                  [-0.009284374637613039, -0.009664883625423075, -0.009406089929318929, -0.009275569312222474,
                   -0.009008037180482098,
                   -0.009504543118853646, -0.009038387119898062, -0.010085811650299970, -0.010262384043848514,
                   0.08553008061795979]],
                 marks=pytest.mark.llvm_not_implemented),
...
derivative_out_test_data = [
    (pnl.Logistic, logistic_helper, {kw.GAIN: RAND1, kw.X_0: RAND2, kw.OFFSET: RAND3, kw.SCALE: RAND4},
     RAND1 * RAND4 * logistic_helper * (1 - logistic_helper)),
    (pnl.ReLU, relu_helper, {kw.GAIN: RAND1, kw.BIAS: RAND2, kw.LEAK: RAND3},
     np.where((test_var - RAND2) > 0, RAND1, RAND1 * RAND3)),
...
@pytest.mark.transfer_function
@pytest.mark.benchmark
@pytest.mark.parametrize("func, variable, params, expected", derivative_out_test_data,
                         ids=lambda x: getattr(x, 'name', None) or getattr(x, 'get', lambda p, q: None)(kw.OUTPUT_TYPE,
                                                                                                        None))
